...
abstract class OAuth2Backend extends ServiceRegistry
{
    ...
    {
        if (
            $id && ($registry = static::allRegistered())
            && isset($registry[$id])
        )
            return $registry[$id];
    }
    ...
    static function process($username, $password = null, &$errors = array())
    {
        ...
        foreach (static::allRegistered() as $bk) {
            ...
            try {
                ...
                if (
                    $result instanceof AuthenticatedUser
                    && ($bk->login($result, $bk))
                )
                    return $result;
                elseif (
                    $result instanceof ClientCreateRequest
...
abstract class StaffAuthenticationBackend  extends AuthenticationBackend
{
    ...
    function login($staff, $bk)
    {
        ...
        if (
            !static::isBackendAllowed($staff, $bk)
    ...
    static function getUser()
    {
        ...
        if (
            !($bk = static::getBackend($id)) //get the backend
...
?>
        <a class="external-sign-in" title="<?php echo $service; ?>" href="login.php?do=ext&amp;bk=<?php echo
    ...
    abstract class UserAuthenticationBackend  extends AuthenticationBackend
    {
        ...
        static function signOut($user)
        {
            ...
            $ost->logDebug(
                _S('User logout'),
                sprintf(
                    _S("%s logged out [%s]" /* Tokens are <username> and <ip> */),
                    $user->getUserName(),
                    $_SERVER['REMOTE_ADDR']
                )
            );
    ...
    implements ExternalAuthentication
    {
        ...
        static $fa_icon = "signin";
        static $sign_in_image_url = 'assets/default/images/sign_in_btn.png';
        static $service_name = "External";
        ...
        function renderExternalLink()
        {
            $service = sprintf('%s %s', __('Sign in with'), $this->getServiceName());
            $imageUrl = ROOT_PATH . static::$sign_in_image_url;
            ?>
        <a class="external-sign-in" title="<?php echo $service; ?>" href="login.php?do=ext&amp;bk=<?php echo urlencode($this->getBkId()); ?>">
            <?php if ($imageUrl) { ?>
                <img class="sign-in-image" src="<?php echo $imageUrl; ?>" alt="<?php echo $service; ?>" />
            <?php } else { ?>
                <div class="external-auth-box">
                    <span class="external-auth-icon">
                        <i class="icon-<?php echo static::$fa_icon; ?> icon-large icon-fixed-with"></i>
    ...
    class StaffAuthStrikeBackend extends  AuthStrikeBackend
    {
        ...
        static function authstrike($credentials)
        {
            ...
            if ($authsession['strikes'] > $cfg->getStaffMaxLogins()) {
                ...
                $ost->logWarning(
                    sprintf(_S('Excessive login attempts (%s)'), $username),
                    $alert,
                    $admin_alert
                );
                ...
                $alert = _S('Username') . ": {$username}\n"
                    ...
                    . _S('Attempts') . ": {$authsession['strikes']}";
                $ost->logWarning(
                    sprintf(_S('Failed agent login attempt (%s)'), $username),
                    $alert,
                    false
                );
            }
 ...
 */
    ...
    class PasswordResetTokenBackend extends StaffAuthenticationBackend
    {
        ...
        function signOn($errors = array())
        {
            ...
            elseif (
                !($id = $_config->get($_POST['token']))
                || $id != $staff->getId()
            )
                $errors['msg'] = __('Invalid reset token');
            elseif (
                !($ts = $_config->lastModified($_POST['token']))
 ...
 */
    ...
    class AccessLinkAuthentication extends UserAuthenticationBackend
    {
        ...
        function _getTicketUser($ticket, $user)
        {
            ...
            // Collaborator?
            elseif (!($user = Collaborator::lookup(
                array(
                    'user_id' => $user->getId(),
    ...
    class ClientPasswordResetTokenBackend extends UserAuthenticationBackend
    {
        ...
        function signOn($errors = array())
        {
            ...
            elseif (!($_config = new Config('pwreset')))
                return false;
            elseif (
                !($acct = ClientAccount::lookupByUsername($_POST['userid']))
                || !$acct->getId()
