...
if ($ost->isUpgradePending()) {
    ...
    if (!strcasecmp(basename(CONFIG_FILE), 'settings.php')) {
        $sysnotice = sprintf(
            __('Please rename config file include/%s to include/ost-config.php to avoid possible conflicts'),
        ...
        if (($perms & 0x0002) || ($perms & 0x0010)) {
            $sysnotice = sprintf(
                __('Please change permission of config file (%1$s) to remove write access. e.g <i>chmod 644 %2$s</i>'),
                basename(CONFIG_FILE),
                basename(CONFIG_FILE)
            );
        }
...
//Page title.
$ost->setPageTitle(__('NEXIIOT :: Admin Control Panel'));
